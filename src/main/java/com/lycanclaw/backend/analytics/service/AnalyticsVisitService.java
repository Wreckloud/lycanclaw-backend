...
public class AnalyticsVisitService {
    ...
    /**
     * 创建一次前台访问记录；非核心路径会被拒绝，避免污染统计数据。
     */
    public VisitStartResponse start(VisitStartRequest request, HttpServletRequest servletRequest) {
        String path = pathPolicy.normalizePath(request == null ? "" : request.path());
        ...
        entity.setPageType(pathPolicy.inferPageType(path, request == null ? "" : request.pageType()));
