...
```powershell
cd D:\Portfolio\Website\LycanClaw\frontend
pnpm run build

cd D:\Portfolio\Website\LycanClaw\backend\deploy
...
```powershell
cd D:\Portfolio\Website\LycanClaw\backend\deploy\scripts
.\up.ps1 -Build
.\down.ps1
```
...
- `WALINE_MAIL_TEMPLATE_ADMIN`

`MAIL_SUBJECT` / `MAIL_TEMPLATE` 负责评论者收到回复时的邮件；`MAIL_SUBJECT_ADMIN` / `MAIL_TEMPLATE_ADMIN` 负责博主收到新评论时的提醒。模板留空时使用 Waline 默认模板。修改后重启 Waline。管理端只显示配置状态和配置片段，不读取或展示 SMTP 密码。
